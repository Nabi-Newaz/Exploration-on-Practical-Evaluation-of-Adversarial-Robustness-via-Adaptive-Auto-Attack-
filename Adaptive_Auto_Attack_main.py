...
class ImageSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        suc = int(self.df.iloc[item]['not_suc'])
        need_atk = int(self.df.iloc[item]['need_atk'])
        sample = {
            'img': image,
            'lab': label_idx,
            'not_suc':suc,
...
def margin_loss(logits,y,i=0,MT=True):
    ...
    T = torch.eye(logits.shape[1]).to(logits.device)[y] * 999999
    TA = (logits-T)
...
def Adaptive_Auto_white_box_attack(model, device, eps, is_random, batch_size, average_num, model_name, data_set="cifar10",
    ...
    not_suc_index = np.ones(len(all_atk_labs)).astype(np.bool_)
    ...
    sorted_logits_index = np.zeros((ori_need_atk_num,logits_num),dtype=np.int64)
    natural_acc = 0
    begin_adv_acc = 0
    No_epoch = -2
    total_bn = 0
    ...
    for out_re in trange(out_restart_num):
        ...
        for r in range(restart_num):
            ...
            print(acc_curve)
    return {
        "clean_acc": float(clean_acc.detach().cpu()) if hasattr(clean_acc, 'detach') else float(clean_acc),
        "acc_curve": [float(x) for x in acc_curve],
        "total_backward": int(used_backward_num),
        "total_forward": int(total_fn),
    }
...
def main(flag, model_name, ep=8./255, random=True, batch_size=128, average_number=100, eps_override=None):
    ...
    if model_name=="TRADES_mnist":
        ...
        ep = eps_override if eps_override is not None else 0.3
        batch_size=512
    ...
    model.to("cuda")
    model.eval()
    result = Adaptive_Auto_white_box_attack(model, device, ep, random, batch_size, average_number, model_name, data_set=data_set, Lnorm=Lnorm)

    # save to a per-model JSON; tag with epsilon so sweep runs don't overwrite each other
    import json, os
    os.makedirs("results", exist_ok=True)
    eps_tag = f"{ep:.4f}".replace(".", "p")  # e.g. 0.3 -> "0p3000"
    out_path = f"results/{model_name}_eps{eps_tag}.json"
    with open(out_path, "w") as f:
        json.dump({"model_name": model_name, "dataset": data_set, "epsilon": ep, **result}, f, indent=2)
...
if __name__ == "__main__":
    # Epsilon sweep on TRADES_mnist (the only checkpoint we have).
    # Default for MNIST in the paper is 0.3. Sweeping below and above
    # to characterize the defense's robustness boundary.
    eps_sweep = [0.1, 0.2, 0.3, 0.35, 0.4]

    for eps in eps_sweep:
        out_path = f"results/TRADES_mnist_eps{f'{eps:.4f}'.replace('.', 'p')}.json"
        if os.path.exists(out_path):
            print(f"[skip] eps={eps} already done -> {out_path}")
            continue
        print(f"\n========== eps = {eps} ==========")
        try:
            ...
            print(f"[fail] eps={eps}: {type(e).__name__}: {e}")
